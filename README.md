...
- [三级缓存架构](#三级缓存架构：)
- [“分发层 应用层” 双层 Nginx 架构](#"分发层 应用层" 双层 Nginx 架构)

[二、项目整体架构](#二、项目整体架构)

- [详情页系统整体架构图](#详情页系统整体架构图：)

[三、项目结构](#三、项目结构)

[四、项目工作流程分析](#四、项目工作流程分析)

- [1、库存服务](#1、库存服务)
	- [库存系统架构图](#库存系统架构图：)
	- [库存系统简要介绍图](#库存系统简要介绍图：)
	- [库存系统具体工作流程图](#库存系统具体工作流程图：)
- [2、数据源服务](#2、数据源服务)
	- [数据源服务工作流程图](#数据源服务工作流程图：)
- [3、缓存服务](#3、缓存服务)
- [4、热点计算服务](#4、热点计算服务)

[五、项目解决的关键问题](#五、项目解决的关键问题)

- [1、解决了 MySQL 和 Redis 的双写最终一致性问题](#1、解决了 MySQL 和 Redis 的双写最终一致性问题)
	- [1.1 问题描述](#1.1 问题描述：)
	- [1.2 问题解决](#1.2 问题解决：)
	- [1.3 更新的时候，为什么要删除缓存，而不是更新缓存？](#1.3 更新的时候，为什么要删除缓存，而不是更新缓存？)
- [2、使用 ZooKeeper 实现分布式锁，解决分布式缓存重建并发冲突问题](#2、使用 ZooKeeper 实现分布式锁，解决分布式缓存重建并发冲突问题)
	- [2.1 问题描述](#2.1 问题描述：)
		- [图示缓存重建并发冲突问题：](#图示缓存重建并发冲突问题：)
	- [2.2 问题解决](2.2 问题解决：)
		- [缓存重建线程工作流程图：](#缓存重建线程工作流程图：)
- [3、通过缓存预热，解决 Redis 冷启动问题](#3、通过缓存预热，解决 Redis 冷启动问题)
	- [为什么要防止冷启动？](#为什么要防止冷启动？)
	- [缓存预热流程图](#缓存预热流程图：)
	- [3.1 使用 Storm 完成实时热数据统计](#3.1 使用 Storm 完成实时热数据统计)
	- [3.2 使用双重 zk 分布式锁完成并行缓存预热](#3.2 使用双重 zk 分布式锁完成并行缓存预热)
- [4、解决热点缓存自动降级与恢复](#4、解决热点缓存自动降级与恢复)
	- [4.1 问题描述](#4.1 问题描述：)
	- [4.2 问题解决](#4.2 问题解决：)
- [5、使用 Hystrix 解决缓存雪崩](#5、使用 Hystrix 解决缓存雪崩)
	- [5.1 问题描述](#5.1 问题描述：)
	- [5.2 问题解决](#5.2 问题解决：)
- [6、解决高并发下的缓存穿透](#6、解决高并发下的缓存穿透)
- [7、解决高并发下的缓存失效](#7、解决高并发下的缓存失效)
...
- 对于**时效性要求高**的数据：
  - 这些数据种类较少，例如：商品库存数据等；
  - **解决措施**：相关库存数据每次发生变更时，直接采取 **MySQL 和 Redis 双写**的方案，保证了MySQL 和 Redis 的**一致性**，并实现了一个**内存队列线程池**，并实现了同一**请求过滤**功能，保证缓存的时效性最高。

- 对于**时效性要求不高**的数据：
  - 这些数据种类繁多，且来自多种不同的系统，例如：商品信息数据、店铺信息数据等；
  - **解决措施**：
    - 采用**三级缓存架构：**
      - **Nginx本地缓存 + Redis分布式缓存 + EhCache缓存**；
    - 采取 **Kafka 异步通知**：
...
**获取缓存的流程简述**：`Nginx本地缓存 ==> 缓存服务 ==> Redis ==> Ehcache`。

**每一层的作用**：

- **Nginx 本地缓存**：应对**热数据的高并发访问**，以最短的时间向用户返回数据。
- **Redis 分布式缓存**：应对**海量普通数据的高并发访问**，并且利用分布式集群做到高可用。
- **EhCache 缓存**：作为最后的防线，主要用来应对 **Redis 集群大规模崩溃**，防止大量请求直接打到数据库。
...
<img src="README.assets/详情页系统整体架构图.jpg" alt="详情页系统整体架构图" width="800" />
...
该项目有4个服务，分别为：库存服务、数据源服务、缓存服务、热点计算服务

- 库存服务：完成对商品库存信息的所有操作；

- 数据源服务、缓存服务和Storm服务：合力完成对商品基本信息和店铺基本信息的所有操作。
...
<img src="README.assets/库存系统_架构图.jpg" alt="库存系统_架构图" width="730" />

##### 库存系统简要介绍图：

<img src="README.assets/库存系统_简要介绍.jpg" alt="库存系统_简要介绍" width="900" />



##### 库存系统具体工作流程图：

<img src="README.assets/库存系统_具体工作流程图.jpg" alt="库存系统_具体工作流程图" width="1100" />
...
<img src="README.assets/数据源服务_工作流程图.jpg" alt="数据源服务_工作流程图" width="750" />
...
- 应用层 Nginx 发现本地缓存中没有数据时，就采取**类似与分发层 Nginx 的 hash 分发策略**，对 productId 取模，**将同一 productId 的请求固定分发到同一个缓存服务实例中去**。避免多个缓存服务实例同时**更新**同一个缓存了。
...
<img src="README.assets/缓存重建并发冲突问题.jpg" alt="缓存重建并发冲突问题" width="600" />
...
<img src="README.assets/缓存重建线程工作流程.jpg" alt="缓存重建线程工作流程" width="1100" />
...
<img src="README.assets/缓存预热流程图.jpg" alt="缓存预热流程图" width="850" />



#### 3.1 使用 Storm 完成实时热数据统计

- 使用OpenResty (Nginx + lua) 将商品详情页访问请求对应的流量、日志，实时上报到 Kafka中。
- Storm 消费 Kafka 中的数据，实时统计出每个商品的访问次数，访问次数的算法基于 LRU 内存数据结构的存储方案。
- 每个 Storm task 启动的时候，基于 zk 分布式锁，将自己 task id 写入同一个 zk 节点中。
- 每个 Storm task 分别负责完成自己的热数据的统计，每隔一段时间，就遍历一下这个map，维护一个前N个商品的 list，更新这个list。
- 使用一个后台线程，每隔一段时间都将排名前N的热数据 list，同步到 zk 中，存储到这个 Storm task对应的一个znode中去。

#### 3.2 使用双重 zk 分布式锁完成并行缓存预热

此时 Storm 的每个task节点分别已经完成了热数据的统计工作，将热点商品的id存成列表，存储在对应的 taskid 的 zk 节点上，用于缓存预热。

服务重新启动或第一次启动时，进行缓存预热，具体流程如下：

1. 从zk中读取 taskid列表；
2. 依次遍历每个 taskid，**尝试获取分布式锁1次**：
   - 如果获取不到锁：不要等待，直接跳过该 taskid，尝试获取下一个 taskid 的分布式锁。
     - 因为已经有其他服务实例线程在预热该 taskid 了。
   - 如果获取到了锁：
     1. 再循环获取 taskid 对应的**预热状态锁，直到获取到了锁**。
     2. 检查这个 taskid 的**预热状态值**是否被设置过，
        1. 如果被设置过，证明已经该taskId被其他线程被预热过了，就不再预热了。
           - 可以有效**避免重复预热**。
        2. 如果没有被设置过：
           1. 执行预热操作：遍历productid列表，从MySQL查询数据，然后写到EhCache和Reisd中。
...
1. **在 Storm 中实时的算出哪些是热点数据**
   - 使用比较简单的算法：
     - 在某个storm task上算出了1万个商品各自的访问次数，存储在 LRUMap 中。
     - 以较高频率（如：每隔5秒）**遍历LRUMap**，然后排序每个商品访问量，求出访问量排后95%的商品访问量均值。
...
- Redis集群彻底崩溃 ==> 缓存服务的大量请求打到数据源服务去查询MySQL ==> MySQL集群崩溃 ==> 数据源服务崩溃 ==> 缓存服务的所有资源耗费在访问redis和数据源服务上hang死 ==> 缓存服务崩溃 ==> Nginx无法访问缓存服务、Redis和数据源服务，只能基于Nginx本地缓存提供服务，但是缓存过期后，数据就全没了 ==> 最终整个系统崩溃。
...
- 直接返回空数据，并将该空数据写入 Redis 中，防止恶意请求重复访问；
- 有一个**异步监听数据变更的机制**，防止 MySQL 中有了对应数据，但 Redis 还在返回空数据：
  - 在第一次查询到这个数据不存在时，就将这个数据的 id 加入到一个内存队列中，
