...
public class CachePrewarmThread extends Thread {
	...
	public void run() {
		...
		LOGGER.info("[CachePrwarmThread获取到taskid列表] taskidList = " + taskidList);

		// 注意：if块中上了两把锁：taskidLockPath(taskid锁) & taskidStatusLockPath(taskid预热状态锁)
		// 为什么上两把锁？：确保一个taskid对应的热点商品数据只会被缓存服务实例预热一次，避免重复预热。
		if(taskidList != null && !"".equals(taskidList)) {
			// 将原来组装的列表且分为一个个原本的taskid
			...
			for(String taskid : taskidListSplited) {
				...
				LOGGER.info("[CachePrewarmThread获取task的预热状态] " +
						"taskid = " + taskid + ", status = " + taskidStatus);
				...
				if("".equals(taskidStatus)) {
					...
					LOGGER.info("[CachePrewarmThread获取到task的热门商品列表] " +
...
//						ProductInfo productInfo = JSONObject.parseObject(productInfoJSON, ProductInfo.class);
						...
						LOGGER.info("[CachePrwarmThread将商品数据设置到本地缓存中] productInfo = " + productInfo);
						cacheService.saveProductInfo2ReidsCache(productInfo);
						LOGGER.info("[CachePrwarmThread将商品数据设置到redis缓存中] productInfo = " + productInfo);
					}
