...
//		ProductInfo productInfo = JSONObject.parseObject(productInfoJSON, ProductInfo.class);
        ...
        if(existedProductInfo != null) {
            ...
            try {
                ...
                if(date.before(existedDate)) {
                    LOGGER.info("[当前ProductInfo的日期(" + productInfo.getModifiedTime()
        ...
        LOGGER.info("[获取刚保存到本地缓存的商品信息] productInfo = " + cacheService.getProductInfoFromLocalCache(productId));
    ...
    private void processShopInfoChangeMessage(JSONObject messageJSONObject) {
        ...
        if(existedShopInfo != null) {
            ...
            try {
                ...
                if(date.before(existedDate)) {
                    LOGGER.info("[当前ShopInfo的日期(" + shopInfo.getModifiedTime()
                            + ") before 于已存在ShopInfo的日期(" + existedShopInfo.getModifiedTime() + ")]");
                    return;
                }
            } catch (Exception e) {
                e.printStackTrace();
            ...
            LOGGER.info("[当前ShopInfo的日期(" + shopInfo.getModifiedTime()
                    + ") after 于已存在ShopInfo的日期(" + existedShopInfo.getModifiedTime() + ")]");
        } else {
